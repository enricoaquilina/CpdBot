...
def get_auth_token(fb_auth_token, fb_user_id, fb_auth_token2, fb_user_id2, orig):
    ...
    if "error" in fb_auth_token2:
        return {"error": "could not retrieve fb_auth_token2"}
    if "error" in fb_user_id2:
        return {"error": "could not retrieve fb_user_id2"}

    if orig:
        fb_access_token = fb_auth_token
        fb_user_id = fb_user_id
        config.orig = not orig
    else:
        fb_access_token = fb_auth_token2
        fb_user_id = fb_user_id2
        config.orig = not orig
...
def authverif():
    res = get_auth_token(config.fb_access_token, config.fb_user_id,
                         config.fb_access_token2, config.fb_user_id2,
                         config.orig)
    if config.orig:
        user = config.fb_username
    else:
        user = config.fb_username2
    if "error" in res:
        return False
    return True, user
